...
// --accent-color-1 / --accent-color-2 variables in their definitions.
const iconColors = {
...
const header = {
  ...
  totalDebtLabel:           `font-medium whitespace-nowrap text-sm leading-none text-[${COLORS['main-color-1']}]`,
  totalDebtAmount:          `truncate font-bold text-xl/[1.25rem] text-[${COLORS['accent-color-2']}]`,
  ...
  goTodayFloatBtn:          "w-[52px] h-[52px] flex items-center justify-center shrink-0 transition-opacity hover:opacity-80 outline-none select-none",
};
...
const monthPill = {
  ...
  containerStyle: {
    ...
    border: `2px solid ${COLORS['main-color-3']}`,
    height: '52px',
  },
  ...
  btnDisabled: "flex items-center justify-center shrink-0 outline-none select-none cursor-default",
  btnDisabledStyle: {
    width: '52px',
    height: '52px',
  },
...
// ─── MAIN MENU ───────────────────────────────────────────────────────────
const mainMenu = {
  // Uses ModalWrapper — backdropOverlay / backdropAnimation defined in modalBase
...
const modal = {
  ...
  // ─── DELETE CONFIRMATION (used by both expense and calendar modals) ──
  ...
  // ─── GAPS BETWEEN EXPENSE MODAL SECTIONS ────────────────────────────
...
// Shared face style — both front and back start from this base
const _flipFaceBase = {
  position: 'absolute',
  top: 0, left: 0,
...
// Determines which delete-mode type string each context uses.
